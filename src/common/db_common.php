...
function dday($date) {
    $now = time(); // 현재 시간을 초로 변환
    $target = strtotime($date); // 목표 일자를 초로 변환
    $diff = $target - $now; // 두 일자 간의 차이 계산
...
echo 'd-day';
echo dday('2023-05-01');


// --------------------------------- 0424 add 신유진
// 함수명	: pet_list_print_ddaylist
// 기능		: ddaylist 출력
// 파라미터	: Array		&$param_arr
// 리턴값	: Array		$result
//------------------------------------------------
function pet_list_print_ddaylist( &$param_arr ) {
    $sql =
        " SELECT "
        ."      * "
        ." FROM "
...
//     $end_date = substr($pet_list_list['list_end'], 0 , 10 );
//     $to_date = date("Y-m-d");
//     if ( $end_date < $to_date ) 
//     {
...
//     }
// }
// var_dump(pet_list_print_pet_name(pet_list_list()));

//------------------------------------------------------------------------------



// function pet_list_dday_diff( ) {
//     $pet_list_list = pet_list_list();
